...
const spotifyClient = (api, token) => endpoint =>
  fetch(`${api}${endpoint}`, {
    headers: {
...
const spotify = spotifyClient(cfg.SPOTIFY_API, cfg.SPOTIFY_TOKEN);

const user = endpoint => gh(`/users/${cfg.GITHUB_OWNER}${endpoint}`);
const repo = endpoint =>
  gh(`/repos/${cfg.GITHUB_OWNER}/${cfg.GITHUB_REPO}${endpoint}`);
const forRecentSongs = limit =>
  spotify(`/me/player/recently-played?type=track&limit=${limit}`);
